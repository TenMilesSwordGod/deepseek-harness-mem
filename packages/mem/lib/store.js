...
export class MemoryStore {
    ...
    constructor(dbPath) {
        ...
        const current = this.#db.prepare('SELECT value FROM mem_meta WHERE key = ?').get('schema_version');
        ...
        this.#insert = this.#db.prepare('INSERT INTO memories (id, content, tags, scope, project, session_id, embedding, dims, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
        this.#selectScope = this.#db.prepare('SELECT id, content, tags, scope, project, session_id, embedding, dims, created_at FROM memories WHERE scope = ? ORDER BY created_at DESC');
        this.#updateEmbedding = this.#db.prepare('UPDATE memories SET embedding = ?, dims = ?, updated_at = ? WHERE id = ?');
        this.#stale = this.#db.prepare('SELECT id, content, embedding FROM memories WHERE dims != ? ORDER BY created_at ASC');
        this.#deleteId = this.#db.prepare('DELETE FROM memories WHERE id = ?');
        this.#count = this.#db.prepare('SELECT COUNT(*) AS n FROM memories');
        this.#recent = this.#db.prepare('SELECT id, content, tags, scope, project, created_at FROM memories ORDER BY created_at DESC LIMIT 1000');
    }
    /** Read one persisted meta value by key. */
    metaGet(key) {
        const row = this.#db.prepare('SELECT value FROM mem_meta WHERE key = ?').get(key);
        return row?.value ?? null;
    }
    /** Persist one meta value by key (upsert). */
    metaSet(key, value) {
        this.#db.prepare('INSERT INTO mem_meta (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value').run(key, value);
    }
    /** Total row count across every scope. */
    count() {
        return Number(this.#count.get().n);
    }
    /** All rows of one scope axis, newest first (candidates for dedup/search). */
    #candidates(scope, project, dims) {
        const rows = this.#selectScope.all(scope === 'global' ? 'global' : 'project')
            .filter((row) => row.dims === dims);
        if (scope === 'global')
            return rows;
        if (project === null)
            return rows;
        const globalRows = this.#selectScope.all('global').filter((row) => row.dims === dims);
        return [
     ...
     */
    record(content, tags, scope, project, sessionId, embedding, dims, dedupThreshold) {
     ...
     */
    search(embedding, scope, project, dims, limit, minSimilarity) {
        const scored = [];
    ...
    staleCount(dims) {
        return Number(this.#db.prepare('SELECT COUNT(*) AS n FROM memories WHERE dims != ?').get(dims).n);
    }
    /**
     ...
     */
    async reEmbedAll(dims, embed, onProgress, isCancelled) {
        const rows = this.#stale.all(dims);
    ...
    list(scope, project, limit) {
        const rows = this.#recent.all();
