...
export const Config = z.object({
  ...
  pinnedInjectCount: z.number().step(1).min(0).max(20).default(4),
  consolidateMinUseCount: z.number().step(1).min(0).max(1000).default(1),
  consolidateHighUseCount: z.number().step(1).min(1).max(100000).default(50),
  consolidateMaxTokens: z.number().step(64).min(256).max(8192).default(2048),
...
export function resolveConfig(config: Partial<MemConfig>): ResolvedMemConfig {
  ...
  return {
    ...
    pinnedInjectCount: config.pinnedInjectCount ?? 4,
    consolidateMinUseCount: config.consolidateMinUseCount ?? 1,
    consolidateHighUseCount: config.consolidateHighUseCount ?? 50,
    consolidateMaxTokens: config.consolidateMaxTokens ?? 2048,
